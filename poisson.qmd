...
prob_4_llamadas <- dpois(4, lambda)

cat("Probabilidd de recibir exactamente 4 llamadas:", prob_4_llamadas)
...
prob_llamadas_30s <- dpois(0, lambda_30s)

cat("Probabilidad de recibir 0 llamadas en 30 segundos:", prob_llamadas_30s)
...
esperado_hora <- lambda * 60

cat("Número esperado de llamadas en 1 hora", esperado_hora)
...
cat("Probabilidad de recibir entre 8 y 12 llamadas", prob_8_a_12)
```
...
lambda_500 <- tasa_defectos * 5  
prob_8_defectos <- dpois(8, lambda_500)

cat("Probabilidad de tener exactamente 8 defectos:", prob_8_defectos)
...
cat("Probabilidad de tener de 10 a más defectos:", prob_10_o_mas)
...
n_productos <- (lambda_requerido / tasa_defectos) * 100

cat("Productos necesarios:", n_productos)
...
```{r}
nueva_tasa <- 1.2
lambda_500_nuevo <- nueva_tasa * 5

prob_10_o_mas_nuevo <- 1 - ppois(9, lambda_500_nuevo)

cat("Probabilidad de tener 10 o más defectos en 500 productos:", prob_10_o_mas_nuevo)
```
...
prob_3_pacientes <- dpois(3, lambda)
cat("Probabilidad:", prob_3_pacientes)
...
prob_0pacientes <- dpois(0, lambda_2h)

cat("Probabilidad de que no llegue ningun paciente en 2 horas:", prob_0pacientes)
...
prob_superar_8 <- 1 - ppois(8, lambda = lambda_4h)

cat("Probabilidad de superar la capacidad de 8 pacientes:", prob_superar_8)
...
cat("Capacidad mínima:", capacidad_minima)
```
...
tamaños <- c(5, 10, 30, 50)

par(mfrow = c(2, 2))

for (n in tamaños) {
  
  medias <- replicate(n_sim, mean(rpois(n, lambda)))
  
  z <- (medias - lambda) / sqrt(lambda / n)
  
  hist(z, breaks = 30, freq = FALSE, 
       main = paste("n =", n),
       xlab = "Z estandarizado",
       ylab = "Densidad",
       col = "lightblue",
       ylim = c(0, 0.5))
  ...
  #Estadísticas
  cat("\n n =", n)
  cat("\nMedia de Z:", mean(z), "(teórico -> 0)\n")
  cat("\nVarianza de Z:", var(z), "(teórico -> 1)\n")
}

par(mfrow = c(1, 1))

medias_30 <- replicate(n_sim, mean(rpois(30, lambda)))
z_30 <- (medias_30 - lambda) / sqrt(lambda / 30)


x_pois <- 0:20
plot(x_pois, dpois(x_pois, lambda), type = "h", lwd = 5,
     main = "Distribución Original\nPoisson",
     xlab = "x", 
     ylab = "P(X = x)",
     col = "blue")

hist(z_30, breaks = 30, freq = FALSE,
     main = "Distribución de Z\n(n = 30)",
     xlab = "Z", ylab = "Densidad",
     col = "lightgreen")
curve(dnorm(x), add = TRUE, col = "red", lwd = 2)
legend("topright", c("Simulación", "N(0,1)"), 
...
cat("X1 + X2\n")
cat("Media:", mean(suma), "\n")
cat("Varianza:", var(suma), "\n\n")

cat("Poisson\n")
cat("Media:", mean(directa), "\n")
cat("Varianza:", var(directa), "\n\n")


#Gráfica
hist(suma, breaks = 20, col = rgb(0, 0, 1, 0.5), main = "X1 + X2 vs Poisson", 
     xlab = "Valor", freq = FALSE)
hist(directa, breaks = 20, col = rgb(1, 0, 0, 0.5), add = TRUE, freq = FALSE)
legend("topright", c("Suma", "Directa"), fill = c(rgb(0, 0, 1, 0.5), rgb(1, 0, 0, 0.5)))
...
#Estadísticas
cat("Suma de las variables\n")
cat("Media:", mean(suma4), "\n")
cat("Varianza:", var(suma4), "\n\n")

cat("Poisson\n")
cat("Media:", mean(directa4), "\n")
cat("Varianza:", var(directa4), "\n\n")

#Gráfica
hist(suma4, breaks = 30, col = rgb(0, 0, 1, 0.5), main = "Suma de 4 vs Poisson", 
     xlab = "Valor", freq = FALSE)
hist(directa4, breaks = 30, col = rgb(1, 0, 0, 0.5), add = TRUE, freq = FALSE)
legend("topright", c("Suma de 4", "Directa"), fill = c(rgb(0, 0, 1, 0.5), rgb(1, 0, 0, 0.5)))
```
...
Prob_degradacion <- 1 - ppois(manejar, lambda)
cat("La probabilidad de que en un minuto el sistema experimente degradación P(X>25)es de:", Prob_degradacion)
...
cat("La probabilidad de que en un minuto el sistema experimente degradación P(X>50)es de:", Prob_degradacion)
```
...
capacidad_optima <- qpois(1 - prob_deseada, lambda)

cat("La capacidad óptima sería:", capacidad_optima)
```
...
visitantes <- rpois(tiempo, lambda)
degradacion <- visitantes > manejar

minutos_deg <- sum(degradacion)
porcentaje_deg <- (minutos_deg / tiempo) * 100

cat("El porcentaje de tiempo con degradación es:", porcentaje_deg)
...
Prob_sin_e <- 1 - ppois(inventario, lambda)

cat("Probabilidad de quedarse sin existencias (P(X>12)):", Prob_sin_e)
```
...
inventario_optimo <- qpois(1 - P_deseada, lambda)

cat("Debería de tener:", inventario_optimo, "unidades, para tener una probabilidad menor al 5% de que se agoten")
```
...
demandas <- 0:50
probs <- dpois(demandas, lambda)

costo_adq <- inventario * costo_adquisicion


inventario_sobrante <- pmax(0, inventario - demandas)
costo_mant_esperado <- sum(inventario_sobrante * costo_mantenimiento * probs)


ventas_perdidas <- pmax(0, demandas - inventario)
esperanza_ventas_perdidas <- sum(ventas_perdidas * probs)
costo_falt_esperado <- esperanza_ventas_perdidas * costo_faltante


costo_total <- costo_adq + costo_mant_esperado + costo_falt_esperado

cat("Costo total esperado: $", round(costo_total, 2), "/día\n\n")

cat("Unidades sobrantes esperadas:", round(sum(inventario_sobrante * probs), 4), "unidades/día\n")
cat("Ventas perdidas esperadas:   ", round(esperanza_ventas_perdidas, 4), "ventas/día\n\n")
...
niveles_inventario <- 0:30
costos_totales <- numeric(length(niveles_inventario))

for (i in 1:length(niveles_inventario)) {
  Q <- niveles_inventario[i]
  c_adq <- Q * costo_adquisicion
  c_mant <- sum(pmax(0, Q - demandas) * costo_mantenimiento * probs)
  c_falt <- sum(pmax(0, demandas - Q) * costo_faltante * probs)
  costos_totales[i] <- c_adq + c_mant + c_falt
}

inventario_optimo_costo <- niveles_inventario[which.min(costos_totales)]
costo_minimo <- min(costos_totales)

cat("Inventario óptimo:", inventario_optimo_costo, "unidades\n")
cat("Costo mínimo esperado: $", round(costo_minimo, 2), "/día\n\n")
```
