...
```{r}
#Función principal
comparar_distribuciones <- function(r, p, n_sim) {
  
#Simular suma de r geométricas
  suma_geometricas <- replicate(n_sim, {
    sum(rgeom(r, p) + 1)  # +1 porque rgeom cuenta fracasos antes del éxito
  })
  
#Simular binomial negativa
  binomial_negativa <- rnbinom(n_sim, size = r, prob = p) + r
  
#Crear data frame
  df <- data.frame(
    Suma_Geometricas = suma_geometricas,
...
#Preparar datos para el gráfico
  df_l <- df %>%
    pivot_longer(cols = everything(), 
                 ...
                 values_to = "Valor") %>%
    mutate(Distribucion = case_when(
      Distribucion == "Suma_Geometricas" ~ "Suma de Geométricas",
      Distribucion == "Binomial_Negativa" ~ "Binomial Negativa"
    ))
  
#Crear el gráfico
  grafico <- ggplot(df_l, aes(x = Valor, fill = Distribucion)) +
    geom_histogram(position = "identity", alpha = 0.6, bins = 30) +
    scale_fill_manual(values = c("Suma de Geométricas" = "#8b5cf6", 
                                   "Binomial Negativa" = "#10b981")) +
    labs(
      title = "Comparación: Suma de Geométricas con Binomial Negativa",
      subtitle = paste0("r = ", r, ", p = ", p, ", n de simulaciones = ", n_sim),
      x = "Número de ensayos",
      y = "Frecuencia",
      fill = "Distribución"
    ) +
...
Ejemplo <- comparar_distribuciones(3, 0.6, 800)
Ejemplo
```
...
Numeros de éxitos = 8 -> r

X_1 = Número de tiros hasta el primer éxito
X_2 = Número de tiros hasta el segundo éxito
.
.
.
X_r = Número de tiros hasta el r (8ctavo) éxito

Se quieren tener 8 éxitos consecutivos; Y = X_1 + X_2 + ... + X_8
$Y\sim Binomial~Negativa(r=8, p=0.7)$
...
Simulacion <- rnbinom(n_sims, r, p)
T_ES <- mean(Simulacion)

T_ES

cat("En el valor teórico obtuvimos que se espera fallar:", T_E, " veces\nMientras que con la simulación se obtuvo:", T_ES, " veces\nComo se puede apreciar son casi lo mismo")
```
...
p_12 <-  dnbinom(fallos, r, p_e)
cat("La probabilidad de tener exactamente 12 pruebas fallidas antes de tener 4 exitos, es de:", round(p_12, 4), "\nEn procentaje:", round(p_12*100, 2), "%")
```
...
p_20 <- 1 - pnbinom(fallos_20, r, p_e)

cat("la probabilidad de más de 20 fallos:", round(p_20, 4))
...
p_e <- 0.3
r <- 4
Fallos_E <- r * (1-p_e)/ (p_e) 

Costo_E <- 10000 * Fallos_E

cat("El costo esperado en prubas fallidas es de: $", Costo_E)
```
