...
set.seed(2)
nsim_tiempo <- 1000
Tiempo_sim <- runif(nsim_tiempo, 2, 8)

densidad_teorica <- 1 / (8-2)
densidad_sim <- dunif(Tiempo_sim, 2, 8)

hist(Tiempo_sim, 
     breaks = 30,
     freq = FALSE,  # Densidad
     main = "Histograma de Tiempos de Carga",
     xlab = "Tiempo (s)",
     ylab = "Densidad",
     col = "cadetblue3",
     border = "white",
...
percentil_75_simulado <- quantile(Tiempo_sim, 0.75)
cat("Percentil 75 (simulado):", percentil_75_simulado, "segundos\n")

percentil_75_teorico <- 2+0.75 * (8-2)
...
```{r}
Probabilidad_menos4s <- 3-2 / 8-2
cat("La probabilidad de que cargue en 3 o menos segundos:", Probabilidad_menos4s)
```
...
n_est <- 500
muestra_estudiantes <- runif(n_est,70,100)

sup_85 <- length(muestra_estudiantes[muestra_estudiantes>85])/ n_est *100
cat("El porcentaje de estudiantes con calificación superior a 85 es ", round(sup_85,2), "%")
  
```

```{r}

df_calificaciones <- tibble(
  id=1:n_est, 
  calificacion=muestra_estudiantes
) %>% mutate(categoria=case_when(
  calificacion<=79 ~ "Aprobado",
  calificacion>79 & calificacion<=89 ~ "Bien",
  calificacion>89 ~ "Excelente",
  .default = "No considerado"
))
df_calificaciones$categoria<- factor(df_calificaciones$categoria)
cat("La tabla de porcentajes es\n ")
table(df_calificaciones$categoria)*100/n_est

```

```{r}

ggplot(df_calificaciones) +
  geom_histogram(aes(calificacion,fill= categoria), color="black", boundary=79, binwidth = 1)+
  theme_bw()
...
generar_truncada <- function(n) {
  muestra <- c()
  while(length(muestra) < n) {
    valores <- runif(1000, 0, 1)
    valores_validos <- valores[valores > 0.3]
    muestra <- c(muestra, valores_validos)
  }
  return(muestra[1:n])
}
...
muestra_original <- generar_truncada(5000)
hist(muestra_original, 
     breaks = 30,
     freq = FALSE,
...
media_teorica <- (0.3 + 1) / 2
sd_teorica <- sqrt((1 - 0.3)^2 / 12)

cat("Media teórica:", media_teorica, "\n")
cat("SD:", sd_teorica, "\n")
cat("Skewness aproximado: 0 (uniforme es simétrica)\n\n")
...
resultados <- data.frame(
  n = tamanos,
  skewness = numeric(length(tamanos)),
  kurtosis = numeric(length(tamanos))
)
...
for(i in 1:length(tamanos)) {
  ...
  medias <- numeric(num_medias)
  for(j in 1:num_medias) {
    muestra <- generar_truncada(n)
    medias[j] <- mean(muestra)
  }
  
  
  skew <- skewness(medias)
  kurt <- kurtosis(medias)
  resultados$skewness[i] <- skew
  resultados$kurtosis[i] <- kurt
  ...
  x_seq <- seq(0.5, 0.8, length.out = 100)
  media_medias <- media_teorica
  sd_medias <- sd_teorica / sqrt(n)
  curva_normal <- dnorm(x_seq, mean = media_medias, sd = sd_medias)
  lines(x_seq, curva_normal, col = "red", lwd = 2)
  

  text(0.52, max(curva_normal) * 0.9, 
       paste("Skew:", round(skew, 3)), 
       adj = 0, cex = 0.8)
  text(0.52, max(curva_normal) * 0.8, 
       paste("Kurt:", round(kurt, 3)), 
       adj = 0, cex = 0.8)
}
...
cat("\nInterpretación\n")
cat("Skewness cercano a 0 indica simetría (normal)\n")
cat("Kurtosis cercano a 0 indica forma normal\n")
cat("\nCriterios aproximados:\n")
cat("- |Skewness| < 0.5: aproximadamente simétrica\n")
cat("- |Kurtosis| < 1: aproximadamente normal\n")


normalizado <- which(abs(resultados$skewness) < 0.5 & abs(resultados$kurtosis) < 1)
if(length(normalizado) > 0) {
  cat("\n¡La distribución se ve aproximadamente normal a partir de n =", 
      tamanos[normalizado[1]], "!\n")
} else {
  cat("\nSe necesita una muestra mayor para normalización completa.\n")
}
```
...
my_circ<- function(x){sqrt(1-x^2)}
ggplot(df_estimacion) +
  geom_point(aes(x,y, color=tipo))+
  geom_function(fun=my_circ,color="firebrick", xlim=c(0,1), linewidth=1.2) +
