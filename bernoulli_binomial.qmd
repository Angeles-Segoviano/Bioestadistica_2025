...
ggplot(medicina_data, aes(x = k, y = probabilidad)) +
  ...
  geom_vline(xintercept = media_curas, color = "red", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = c(18, 22), color = "orange", linetype = "dotted", linewidth = 1) +
...
# Parámetros
p_defectuosa <- 0.02
p_Nodefectuosa <- 1-p_defectuosa

cat("La probabilidad es de:", p_Nodefectuosa, "\n")
...
# Parámetros
media_d <- p_defectuosa
varianza_d <- p_defectuosa * (p_Nodefectuosa)
SD_d <- sqrt(varianza_d)

cat("Parámetros estadísticos:\n")
cat("Media:", media_d, "\n")
cat("Varianza:", varianza_d, "\n")
cat("Desviación estándar:", SD_d, "\n")
...
p_defectuosa <- 0.02
x <- 2
p_2 <- dbinom(x, n, p_defectuosa)

cat("La probabilidad de encontrar exactamente 2 defectuosas (P(X=2)):", p_2)
```
...
```{r}
Opciones<- 4
p_Respuestacorrecta <- 1/Opciones
p_RespuestaIncorrecta <- 1 - p_Respuestacorrecta
cat("La probabilidad de obtener una respuesta correcta es de ", p_Respuestacorrecta, "mientras que la de tener una incorrecta es de ", p_RespuestaIncorrecta)
```
...
cat("La probabilidad de acertar es de:", p_Respuestacorrecta)
```
...
Opciones<- 4
p_Respuestacorrecta <- 1/Opciones
p_NR <- 1-p_Respuestacorrecta

Media <- p_Respuestacorrecta
Varianza <-p_Respuestacorrecta * (p_NR)
SD <- sqrt(Varianza)
...
p_Ncara <- 1 - p_cara
# a)
p_8c <- dbinom(8, n_monedas, p_cara)

cat("\nLa probabilidad de obtener exactamente 8 caras:", p_8c)

# b) de 12 a 15
p_Almenos12c <-pbinom(15, n_monedas, p_cara) - pbinom(11, n_monedas, p_cara)

cat("\nLa probabilidad de obtener exactamente al menos 12 caras:", p_Almenos12c)

# c) Parámetros estadísticos
media <- n_monedas * p_cara
var <- media * p_Ncara
SD <- sqrt(var)
cat("\n\nParámetros estadísticos:\n")
cat("Media:", media, "\n")
cat("Varianza:", var, "\n")
cat("Desviación estándar:", round(SD, 4), "\n")
...
# a)
x_a <- 18
p_18 <- dbinom(x_a, n, P_Efectividad)

cat("La probabilidad de que se curen exactamente 18 pacientes:", round(p_18, 4))

# b) 
p_15a19c <- pbinom(19, n, P_Efectividad) - pbinom(14, n, P_Efectividad)

cat("\nLa probabilidad de que se curen de 15 a 19 pacientes:", round(p_15a19c, 4))

# c)
E_Curen <- P_Efectividad * n
cat("\nSe espera que se curen:", E_Curen)
...
- $X$ es el número de éxitos en n ensayos de Bernoulli.
- $Y$ es el número de éxitos en m ensayos de Bernoulli.

Entonces; 

- $X$ + $Y$ representa la suma de n ensayos y m ensayos de Bernoulli.

Por lo que $X + Y$ tendría una distribución $bin(n+m, p)$
...
# Calculo
f_xy <- dbinom(X+Y, n + m, p)
f_XY <- dbinom(Z, l, p)

cat("Comprobación:\n")
cat("Tenemos los valores de X, Y y Z que siguen una distribución binomial:\n" ,"X:", X, "n: 25\n Y:", Y, "m: 15\n Z:", Z)
cat("\nZ es la suma de X y Y con l (m+n) ensayos de Bernoulli, l:", l,"\nEntonces hacemos el cálculo con dbinom:\ndbinom(X+Y,n+m,p) y \ndbinom(Z, l, p)\npara comprobar si da el mismo resultado, se utiliza identical()" )
cat("\ndbinom(X+Y,n+m,p):", f_xy, "\ndbinom(Z, l, p):", f_XY, "\nidentical(f(X+Y), f(Z)):", identical(f_xy, f_XY))
...
xy <- X + Y
XY <- rbinom(n_simulaciones, size = n+m, prob = p)
...
# Visualización
par(mfrow = c(1, 1))
hist(XY_data$xy, breaks = 30, probability = TRUE,
     col = rgb(1, 0, 0, 0.5),
     main = "Comparación: X + Y vs Bin(n+m, p)",
     xlab = "Número de éxitos",
     ylab = "Probabilidad",
     xlim = range(c(XY_data$xy, XY_data$XY)))
hist(XY_data$XY, breaks = 30, probability = TRUE,
