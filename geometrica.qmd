...
cat("La probabilidad de necesitar exactamente 5 lanzamientos para obtener la primer cara es de:",P_5)
...
P_m3 <- 1- pgeom(3, P)
cat("\nLa probabilidad de necesitar más de 3 lanzamientos para obtener la primer cara es de:", P_m3, "\nOsea, la probabilidad acumulada de infinitos ensayos - la probabilidad acumulada de tener 3. Pero aquí la estamos calculando usando el complemento, ya que sabemos que la suma de todas las probabilidades nos dará 1. ")
```
...
```{r}
P_Anotar <- 0.7
P_3 <- P_Anotar * ((1 - P_Anotar)^2)

cat("La probabilidad es de:", P_3)
```
...
cat("La esperanza es de:", E_Anotar)
```
...
```{r}
P_25 <- pgeom(4, 0.7) - pgeom(0, 0.7)
a <- 0.21+0.063+0.0189+0.00567

cat("La probabilidad es de:", P_25)
```
...
cat("El numero de ensayos hasta encontrar la primer pieza defectuosa se espera que sea de:", E_i, " ensayos.")
```
...
```{r}
p_defect <- 0.02
P_100 <- dgeom(100, 0.02)

cat("La probabilidad de tener 100 ensayos hasta obtener el primer exito es de:", P_100)
```
...
cat("La probabilidad de s+t:", prob_s_t, "\nLa probabilidad de s:", prob_s, "\nLa probabilidad de t:", prob_t, "\nLa probabilidad condicional:", prob_condicional, "\n\nComo se puede ver, la probabilidad condicional es igual a la probabilidad de t")
