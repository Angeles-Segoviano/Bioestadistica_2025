...
X <- rf(n_simulaciones, df1 = d1, df2 = d2)

X_inverso <- 1/X


Y <- rf(n_simulaciones, df1 = d2, df2 = d1)


par(mfrow = c(1, 2))


hist(X_inverso, probability = TRUE, breaks = 100, 
     xlim = c(0, 10), ylim = c(0, 0.8),
     main = paste0("1/X donde X ~ F(", d1, ", ", d2, ")"),
     xlab = "Valor", col = "lightblue", border = "white")


x_vals <- seq(0.01, 10, length.out = 1000)
lines(x_vals, df(x_vals, df1 = d2, df2 = d1), 
      col = "red", lwd = 2)
legend("topright", legend = c("Simulación 1/X", paste0("PDF F(", d2, ", ", d1, ")")),
       col = c("lightblue", "red"), lwd = c(10, 2))


hist(Y, probability = TRUE, breaks = 100, 
     xlim = c(0, 10), ylim = c(0, 0.8),
     main = paste0("Y ~ F(", d2, ", ", d1, ")"),
     xlab = "Valor", col = "lightgreen", border = "white")
lines(x_vals, df(x_vals, df1 = d2, df2 = d1), 
      col = "red", lwd = 2)
legend("topright", legend = c(paste0("Simulación F(", d2, ", ", d1, ")"), 
                              paste0("PDF F(", d2, ", ", d1, ")")),
       col = c("lightgreen", "red"), lwd = c(10, 2))

cat("  Media:", mean(X_inverso), "\n")
cat("  Varianza:", var(X_inverso), "\n")
cat("\nEstadísticos de F(d2, d1):\n")
cat("  Media:", mean(Y), "\n")
cat("  Varianza:", var(Y), "\n")
cat("\nMedia teórica F(", d2, ", ", d1, "):", d1/(d1-2), "\n")


X2 <- rf(n_simulaciones, df1 = d1, df2 = d2)
Y2 <- (d1 * X2) / (d1 * X2 + d2)


Beta_muestra <- rbeta(n_simulaciones, shape1 = d1/2, shape2 = d2/2)

par(mfrow = c(1, 2))

hist(Y2, probability = TRUE, breaks = 50,
     main = "Transformación de F a Beta",
     xlab = "Valor", col = "lightblue", border = "white")
x_vals2 <- seq(0.01, 0.99, length.out = 1000)
lines(x_vals2, dbeta(x_vals2, shape1 = d1/2, shape2 = d2/2),
      col = "red", lwd = 2)
legend("topright", legend = c("Simulación", paste0("PDF Beta(", d1/2, ", ", d2/2, ")")),
       col = c("lightblue", "red"), lwd = c(10, 2))

hist(Beta_muestra, probability = TRUE, breaks = 50,
     main = paste0("Beta(", d1/2, ", ", d2/2, ")"),
     xlab = "Valor", col = "lightgreen", border = "white")
lines(x_vals2, dbeta(x_vals2, shape1 = d1/2, shape2 = d2/2),
      col = "red", lwd = 2)
legend("topright", legend = c("Simulación Beta", "PDF Beta"),
       col = c("lightgreen", "red"), lwd = c(10, 2))


cat("  Media:", mean(Y2), "\n")
cat("  Varianza:", var(Y2), "\n")
cat("\nEstadísticos de Beta(", d1/2, ", ", d2/2, "):\n")
cat("  Media:", mean(Beta_muestra), "\n")
cat("  Varianza:", var(Beta_muestra), "\n")
cat("\nMedia teórica Beta:", (d1/2)/((d1/2)+(d2/2)), "\n")

Chi1 <- rchisq(n_simulaciones, df = d1)
Chi2 <- rchisq(n_simulaciones, df = d2)

F_from_chi <- (Chi1/d1) / (Chi2/d2)
F_directa <- rf(n_simulaciones, df1 = d1, df2 = d2)


par(mfrow = c(1, 2))

hist(F_from_chi, probability = TRUE, breaks = 100,
     xlim = c(0, 5), ylim = c(0, 0.8),
     main = "F desde Chi-cuadrado",
     xlab = "Valor", col = "lightblue", border = "white")
x_vals3 <- seq(0.01, 5, length.out = 1000)
lines(x_vals3, df(x_vals3, df1 = d1, df2 = d2),
      col = "red", lwd = 2)
legend("topright", legend = c("(Chi1/d1)/(Chi2/d2)", paste0("PDF F(", d1, ", ", d2, ")")),
       col = c("lightblue", "red"), lwd = c(10, 2))

hist(F_directa, probability = TRUE, breaks = 100,
     xlim = c(0, 5), ylim = c(0, 0.8),
     main = paste0("F(", d1, ", ", d2, ") Directa"),
     xlab = "Valor", col = "lightgreen", border = "white")
lines(x_vals3, df(x_vals3, df1 = d1, df2 = d2),
      col = "red", lwd = 2)
legend("topright", legend = c(paste0("Simulación F(", d1, ", ", d2, ")"), "PDF F"),
       col = c("lightgreen", "red"), lwd = c(10, 2))


cat("  Media:", mean(F_from_chi), "\n")
cat("  Varianza:", var(F_from_chi), "\n")
cat("\nEstadísticos de F(", d1, ", ", d2, "):\n")
cat("  Media:", mean(F_directa), "\n")
cat("  Varianza:", var(F_directa), "\n")
cat("\nMedia teórica F(", d1, ", ", d2, "):", d2/(d2-2), "\n")
