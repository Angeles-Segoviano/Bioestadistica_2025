...
resultados <- data.frame(
  n = tamanios,
  media_medias = numeric(length(tamanios)),
  media_teorica = numeric(length(tamanios)),
  var_medias = numeric(length(tamanios)),
  var_teorica = numeric(length(tamanios)),
  diferencia_media = numeric(length(tamanios)),
  diferencia_var = numeric(length(tamanios))
)
...
for(i in 1:length(tamanios)) {
  ...
  var_teorica_medias <- varianza_teorica / n
  
  resultados$media_medias[i] <- media_medias
  resultados$media_teorica[i] <- media_teorica
  resultados$var_medias[i] <- var_medias
  resultados$var_teorica[i] <- var_teorica_medias
  resultados$diferencia_media[i] <- abs(media_medias - media_teorica)
  resultados$diferencia_var[i] <- abs(var_medias - var_teorica_medias)
  ...
  sd_teorica_medias <- sqrt(var_teorica_medias)
  curva_normal <- dnorm(x_seq, mean = media_teorica, sd = sd_teorica_medias)
  lines(x_seq, curva_normal, col = "red", lwd = 2, lty = 1)
  

  legend("topright", 
         legend = c("Medias", "Normal TCL"),
...
print(resultados[, c("n", "media_medias", "media_teorica", "diferencia_media")])

cat("\n\nComparación de Varianzas:\n")
cat("(Varianza teórica de medias = σ²/n)\n")
print(resultados[, c("n", "var_medias", "var_teorica", "diferencia_var")])
...
ggplot(df_exr)+
  geom_histogram(aes(x= Valor, y= after_stat(density)), fill="#EADDCA", color="black", alpha=0.5, bins = 30)+
  geom_function(fun=dgamma, args=list(shape=n, rate = 1.5), xlim= c(0,10.5), linewidth=1.2, color="gold")+
  theme_bw()
...
ggplot(df_exr)+
  geom_histogram(aes(x= Valor, y= after_stat(density)), fill="#EADDCA", color="black", alpha=0.5, bins = 30)+
  geom_function(fun=dgamma, args=list(shape=n, rate = 1.5), xlim= c(0,15.5), linewidth=1.2, color="gold")+
  theme_bw()
...
ggplot(df_exr)+
  geom_histogram(aes(x= Valor, y= after_stat(density)), fill="#EADDDA", color="black", alpha=0.5, bins = 30)+
  geom_function(fun=dgamma, args=list(shape=n, rate = 1.5), xlim= c(0,25.5), linewidth=1.2, color="gold")+
  theme_bw()
