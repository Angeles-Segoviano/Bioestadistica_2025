...
varianza_teorica <- (alpha * beta_param) / ((alpha + beta_param)^2 * (alpha + beta_param + 1))

cat("α:", alpha, "\n")
cat("β:", beta_param, "\n")
cat("Media teórica (μ):", media_teorica, "\n")
cat("Varianza teórica (σ²):", varianza_teorica, "\n")
cat("SD (σ):", sqrt(varianza_teorica), "\n\n")

tamanos <- seq(100, 10000, by = 100)


muestra_completa <- rbeta(max(tamanos), shape1 = alpha, shape2 = beta_param)


medias_acumulativas <- numeric(length(tamanos))
varianzas_acumulativas <- numeric(length(tamanos))

for(i in 1:length(tamanos)) {
  n <- tamanos[i]
  muestra_parcial <- muestra_completa[1:n]
  medias_acumulativas[i] <- mean(muestra_parcial)
  varianzas_acumulativas[i] <- var(muestra_parcial)
}
...
plot(tamanos, medias_acumulativas,
     type = "l",
     col = "blue",
     lwd = 2,
     xlab = "Tamaño de muestra (n)",
     ylab = "Media muestral",
     main = "Convergencia de la Media a μ teórica",
     ylim = c(min(medias_acumulativas, media_teorica) - 0.01,
              max(medias_acumulativas, media_teorica) + 0.01))


abline(h = media_teorica, col = "red", lwd = 2, lty = 2)


epsilon <- 1.96 * sqrt(varianza_teorica) / sqrt(tamanos)
lines(tamanos, media_teorica + epsilon, col = "gray", lty = 3)
lines(tamanos, media_teorica - epsilon, col = "gray", lty = 3)

legend("topright",
       legend = c("Media muestral", "Media teórica", "IC 95%"),
       col = c("blue", "red", "gray"),
       lwd = c(2, 2, 1),
       lty = c(1, 2, 3),
       cex = 0.8)
...
plot(tamanos, varianzas_acumulativas,
     type = "l",
     col = "darkgreen",
     lwd = 2,
     xlab = "Tamaño de muestra (n)",
     ylab = "Varianza muestral",
     main = "Convergencia de la Varianza a σ² teórica",
     ylim = c(min(varianzas_acumulativas, varianza_teorica) - 0.002,
              max(varianzas_acumulativas, varianza_teorica) + 0.002))


abline(h = varianza_teorica, col = "red", lwd = 2, lty = 2)

legend("topright",
       legend = c("Varianza muestral", "Varianza teórica"),
       col = c("darkgreen", "red"),
       lwd = c(2, 2),
       lty = c(1, 2),
       cex = 0.8)
...
muestra_grande <- muestra_completa[1:10000]

hist(muestra_grande,
     breaks = 50,
     freq = FALSE,
     main = "Histograma con n = 10,000",
     xlab = "Valor",
     ylab = "Densidad",
     col = "lightpink",
     border = "white",
...
pdf_teorica <- dbeta(x_seq, shape1 = alpha, shape2 = beta_param)
lines(x_seq, pdf_teorica, col = "red", lwd = 3)

legend("topright",
       legend = c("Datos simulados", "PDF teórica Beta(2,5)"),
       fill = c("lightpink", NA),
       border = c("white", NA),
       lty = c(NA, 1),
       col = c(NA, "red"),
...
error_media <- abs(medias_acumulativas - media_teorica)

plot(tamanos, error_media,
     type = "l",
     col = "purple",
     lwd = 2,
     xlab = "Tamaño de muestra (n)",
     ylab = "Error absoluto",
     main = "Error Absoluto de la Media",
     log = "y")  
...
indices_seleccionados <- c(1, 10, 25, 50, 75, 100)  # n = 100, 1000, 2500, 5000, 7500, 10000
resultados <- data.frame(
  n = tamanos[indices_seleccionados],
  Media_muestral = medias_acumulativas[indices_seleccionados],
  Media_teorica = media_teorica,
  Error_media = abs(medias_acumulativas[indices_seleccionados] - media_teorica),
  Var_muestral = varianzas_acumulativas[indices_seleccionados],
  Var_teorica = varianza_teorica,
  Error_var = abs(varianzas_acumulativas[indices_seleccionados] - varianza_teorica)
)

cat("\nTabla de Convergencia:\n")
print(resultados, digits = 5)

cat("La media muestral converge a μ =", media_teorica, "\n")
cat("Error final (n=10000):", abs(medias_acumulativas[length(medias_acumulativas)] - media_teorica), "\n\n")

cat("La varianza muestral converge a σ² =", varianza_teorica, "\n")
cat("Error final (n=10000):", abs(varianzas_acumulativas[length(varianzas_acumulativas)] - varianza_teorica), "\n\n")

cat("El histograma con n=10000 se ajusta muy bien a la PDF teórica,\n")
cat("confirmando la Ley de Grandes Números.\n\n")

cat("El error disminuye aproximadamente como 1/√n (escala log en gráfico 4).\n")
```
...
y_beta <- dbeta(x_beta, shape1 = alpha, shape2 = beta_param)

plot(x_beta, y_beta,
     type = "l",
     lwd = 2,
     col = "darkblue",
     main = "Distribución Original Beta(2,5)",
     xlab = "x",
     ylab = "Densidad",
     xlim = c(0, 1))
polygon(c(x_beta, rev(x_beta)), c(y_beta, rep(0, length(y_beta))),
        col = rgb(0, 0, 1, 0.2), border = NA)


resultados <- data.frame(
  n = tamanos,
  media_medias = numeric(length(tamanos)),
  sd_medias = numeric(length(tamanos)),
  sd_teorica = numeric(length(tamanos))
)
...
for(i in 1:length(tamanos)) {
  ...
  for(j in 1:num_muestras) {
    muestra <- rbeta(n, shape1 = alpha, shape2 = beta_param)
    medias[j] <- mean(muestra)
  }
  
  media_medias <- mean(medias)
  sd_medias <- sd(medias)
  sd_teorica_medias <- sqrt(varianza_teorica / n)

  
  resultados$media_medias[i] <- media_medias
  resultados$sd_medias[i] <- sd_medias
  resultados$sd_teorica[i] <- sd_teorica_medias
  ...
  curva_normal <- dnorm(x_seq, mean = media_teorica, sd = sd_teorica_medias)
  lines(x_seq, curva_normal, col = "red", lwd = 2)
  
  abline(v = media_teorica, col = "blue", lwd = 2, lty = 2)
  
}

n_final <- 100
medias_final <- numeric(num_muestras)
for(j in 1:num_muestras) {
  muestra <- rbeta(n_final, shape1 = alpha, shape2 = beta_param)
  medias_final[j] <- mean(muestra)
}
