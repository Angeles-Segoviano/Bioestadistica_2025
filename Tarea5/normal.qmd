...
Muestra_estudiantes <- rnorm(n, mu, SD)

Media_m <- mean(Muestra_estudiantes)
Mediana_m <- median(Muestra_estudiantes)
SD_m <- sd(Muestra_estudiantes)

cat("Parámetros Estadísticos:\n Media: ", Media_m, "\n Mediana: ", Mediana_m, "\n SD: ", SD_m)

Prob_estudiantes_mas180 <- 1 - pnorm(180, mu, SD)
Porcentaje_em180 <- Prob_estudiantes_mas180 * 100

cat("\nAproximadamente, el ", Porcentaje_em180, "% miden más de 180 cm")

percentil_25 <- qnorm(0.25, mu, SD)

cat("\nLa altura del estudiante en el percentil 25 es de ", percentil_25, "cm")


hist(Muestra_estudiantes, 
     breaks = 30,
     freq = FALSE,  
     main = "Histograma de Alturas con Densidad Normal Teórica",
     xlab = "Altura (cm)",
     ylab = "Densidad",
     col = "lightblue",
     border = "black")
...
x <- seq(min(Muestra_estudiantes) - 10, max(Muestra_estudiantes) + 10, length = 200)
y <- dnorm(x, mu, SD)
...
legend("topright", 
       legend = c("Datos simulados", "Curva normal teórica", "Media (μ=170)"), 
...
Dias_prod <- rnorm(n_t, mu_temp, SD_temp)


porcentaje_fuera <- (1 - (pnorm(limite_sup, mu_temp, SD_temp) - pnorm(limite_inf, mu_temp, SD_temp))) * 100

cat("Porcentaje del tiempo en el que la temperatura está fuera de lo especificado: ", porcentaje_fuera, "%")

mu_temp_nueva <- 355

porcentaje_fuera_nueva <- (1 - (pnorm(limite_sup, mu_temp_nueva, SD_temp) - pnorm(limite_inf, mu_temp_nueva, SD_temp))) * 100

cat("\nReducción en calidad en: ", round((porcentaje_fuera - porcentaje_fuera_nueva), 2), "puntos porcentuales.")


z_critico <- qnorm(0.0005)
sigma_necesaria <- (limite_inf - mu_temp) / z_critico

cat("\nZ crítico:", z_critico)
cat("\nDesviación estándar necesaria (μ=350):", round(sigma_necesaria, 3), "°C")
...
```
```{r}
prob_menor30mil <- pnorm(30000, media_exr, sd_exr)
cat("Probabilidad ventas < 30000 =", prob_menor30mil)
cat("\nSe espera que en", 12 * round(prob_menor30mil, 2), " meses al año se tengan ventas menores a 30 mil.")

ggplot()+
  geom_function(fun = dnorm, args =list(mean =media_exr, sd = sd_exr), xlim = c(2000, 98000), color ="firebrick", linewidth = 1.2)+
  stat_function(fun = dnorm, args =list(mean =media_exr, sd = sd_exr), geom = "area", xlim = c(0, 30000), fill ="olivedrab3", linewidth = 1.2, alpha = 0.5)+
  annotate(geom ="label", x = 30000, y= 0.00001, label = paste("P(Ventas<30000)=", round(pnorm(30000, media_exr, sd_exr),5)))+
  theme_bw()
```
```{r}
Q1 <- qnorm(0.25, media_exr, sd_exr)
Q3 <- qnorm(0.75, media_exr, sd_exr)

IQR_ventas <- Q3 - Q1

cat("El rango intercuartílico de las ventas es de: $", round(IQR_ventas, 2))
```
```{r}
prob_mayor45mil <- pnorm(45000, media_exr, sd_exr, lower.tail = FALSE)
porcentaje_cumple <- prob_mayor45mil * 100
cat("El porcentaje de meses en el que las ventas son mayores a 45 mil es de:", porcentaje_cumple, "%")
...
baterias <- rnorm(n, media_bat, SD_bat)

garantia_95 <- qnorm(0.05, media_bat, SD_bat)

cat("Garantía teórica (percentil 5):", round(garantia_95, 2), "horas")
cat("\nInterpretación: El 95% de las baterías duran más de", 
    round(garantia_95, 2), "horas")

prob_falla_900 <- pnorm(900, media_bat, SD_bat)
n_vendidas <- 10000
fallas_esperadas <- prob_falla_900 * n_vendidas

cat("\nDe 10,000 baterías vendidas, aproximadamente:", round(fallas_esperadas), "baterías fallarán\n\n")

media_muestral <- mean(baterias)
mediana_muestral <- median(baterias)
media_teorica <- media_bat
mediana_teorica <- media_bat  

cat("Media:")
cat("\n  Teórica:", media_teorica, "horas")
cat("\n  Muestral:", round(media_muestral, 2), "horas")
cat("\n  Diferencia:", round(media_muestral - media_teorica, 2), "horas")

cat("Mediana:")
cat("\n  Teórica:", mediana_teorica, "horas")
cat("\n  Muestral:", round(mediana_muestral, 2), "horas")
cat("\n  Diferencia:", round(mediana_muestral - mediana_teorica, 2), "horas")

percentil_10 <- qnorm(0.10, media_bat, SD_bat)
percentil_10_muestral <- quantile(baterias, 0.10)

cat("\nPercentil 10 (teórico):", round(percentil_10, 2), "horas")
cat("\nPercentil 10 (muestral):", round(percentil_10_muestral, 2), "horas\n\n")
...
errores <- rnorm(n, media_error, SD_error)

prob_entre_5 <- pnorm(5, media_error, SD_error) - pnorm(-5, media_error, SD_error)
porcentaje_entre_5 <- prob_entre_5 * 100

cat("Porcentaje teórico:", round(porcentaje_entre_5, 2), "%")


errores_entre_5 <- sum(errores >= -5 & errores <= 5)
porcentaje_muestral_5 <- (errores_entre_5 / n) * 100
cat("\nMuestra:\n")
cat("  Mediciones entre -5 y +5:", errores_entre_5, "de", n)
cat("\n  Porcentaje:", round(porcentaje_muestral_5, 2), "%")

prob_mayor_6_abs <- 2 * pnorm(-6, media_error, SD_error)
porcentaje_mayor_6 <- prob_mayor_6_abs * 100

cat("\nProbabilidad P(|Error| > 6):", round(prob_mayor_6_abs, 6))
cat("\nPorcentaje:", round(porcentaje_mayor_6, 4), "%")

errores_mayor_6_abs <- sum(abs(errores) > 6)
porcentaje_muestral_6 <- (errores_mayor_6_abs / n) * 100
cat("\nMuestra:\n")
cat("  Mediciones con |error| > 6:", errores_mayor_6_abs, "de", n)
cat("\n  Porcentaje:", round(porcentaje_muestral_6, 2), "%")

prob_preciso <- pnorm(3, media_error, SD_error) - pnorm(-3, media_error, SD_error)
porcentaje_preciso <- prob_preciso * 100

cat("\nProbabilidad P(|Error| ≤ 3):", round(prob_preciso, 6))
cat("\nPorcentaje de mediciones precisas:", round(porcentaje_preciso, 2), "%")

mediciones_precisas <- sum(abs(errores) <= 3)
porcentaje_muestral_preciso <- (mediciones_precisas / n) * 100
cat("Muestra:\n")
cat("  Mediciones precisas:", mediciones_precisas, "de", n)
cat("\n  Porcentaje:", round(porcentaje_muestral_preciso, 2), "%")

media_muestral <- mean(errores)
mediana_muestral <- median(errores)
...
cat("   Mediana muestral:", round(mediana_muestral, 4), "\n")
cat("   Diferencia:", round(abs(media_muestral - mediana_muestral), 4), "\n")

cat("* Coeficiente de asimetría (Skewness):\n")
cat("   Valor:", round(asimetria, 4), "\n")

cat("* Percentiles simétricos:\n")
p10 <- quantile(errores, 0.10)
p90 <- quantile(errores, 0.90)
p25 <- quantile(errores, 0.25)
p75 <- quantile(errores, 0.75)

cat("   P10:", round(p10, 3), "   P90:", round(p90, 3))
cat("   P25:", round(p25, 3), "   P75:", round(p75, 3))
...
curve(dnorm(x, media_error, SD_error), 
      col = "darkblue", 
      lwd = 2, 
      add = TRUE)
...
calif_A <- rnorm(n_A, media_A, SD_A)
calif_B <- rnorm(n_B, media_B, SD_B)

prob_A_supera_mediaB <- pnorm(82, media_A, SD_A, lower.tail = FALSE)
porcentaje_A_supera <- prob_A_supera_mediaB * 100

cat("Probabilidad teórica P(Calif_A > 82):", round(prob_A_supera_mediaB, 6), "\n")
cat("Porcentaje teórico:", round(porcentaje_A_supera, 2), "%\n\n")

estudiantes_A_supera <- sum(calif_A > 82)
porcentaje_muestral_A <- (estudiantes_A_supera / n_A) * 100
cat("Muestra:\n")
cat("  Estudiantes del grupo A con calificaciones > 82:", estudiantes_A_supera, "de", n_A)
cat("\n  Porcentaje:", round(porcentaje_muestral_A, 2), "%\n\n")
cat("  Solo el", round(porcentaje_A_supera, 2), "% del Grupo A supera\n")


prob_B_menor_70 <- pnorm(70, media_B, SD_B)
porcentaje_B_menor_70 <- prob_B_menor_70 * 100

cat("Probabilidad teórica P(Calificacion grupo B < 70):", round(prob_B_menor_70, 6))
cat("\nPorcentaje teórico:", round(porcentaje_B_menor_70, 4), "%\n\n")

estudiantes_B_menor70 <- sum(calif_B < 70)
porcentaje_muestral_B <- (estudiantes_B_menor70 / n_B) * 100
cat("Muestra:\n")
cat("  Estudiantes del grupo B con calificacion < 70:", estudiantes_B_menor70, "de", n_B)
cat("\n  Porcentaje:", round(porcentaje_muestral_B, 2), "%\n\n")
cat("  Es muy poco probable (", round(porcentaje_B_menor_70, 4), 
    "%) que un estudiante del grupo B obtenga menos de 70 puntos\n")


datos <- data.frame(
  calificacion = c(calif_A, calif_B),
  grupo = rep(c("Grupo A", "Grupo B"), times = c(n_A, n_B))
)

ggplot(datos, aes(x = calificacion, fill = grupo)) +
  geom_histogram(alpha = 0.6, position = "identity", bins = 30, color = "black") +
  scale_fill_manual(values = c("Grupo A" = "#E69F00", "Grupo B" = "#56B4E9")) +
  labs(title = "Comparación de Calificaciones entre Grupos",
       x = "Calificación",
       y = "Frecuencia",
       fill = "Grupo") +
  theme_minimal() +
...
percentil_90_A <- qnorm(0.90, media_A, SD_A)
percentil_90_B <- qnorm(0.90, media_B, SD_B)

cat("Grupo A:\n")
cat("  Percentil 90 (teórico):", round(percentil_90_A, 2), "puntos")
cat("\n  El 10% superior del grupo A tiene ≥", 
    round(percentil_90_A, 2), "puntos\n\n")

cat("Grupo B:\n")
cat("  Percentil 90 (teórico):", round(percentil_90_B, 2), "puntos")
cat("\n  El 10% superior del grupo B tiene ≥", 
    round(percentil_90_B, 2), "puntos\n\n")

percentil_90_A_muestral <- quantile(calif_A, 0.90)
percentil_90_B_muestral <- quantile(calif_B, 0.90)
cat("Muestra:\n")
cat("  Grupo A (muestral):", round(percentil_90_A_muestral, 2), "puntos")
cat("\n  Grupo B (muestral):", round(percentil_90_B_muestral, 2), "puntos\n\n")
cat("  Diferencia en percentil 90:", 
    round(percentil_90_B - percentil_90_A, 2), "puntos")
cat("\n  El 10% superior del grupo B supera al del grupo A\n\n")
