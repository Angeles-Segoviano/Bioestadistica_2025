...
p_casos <- ggplot() +
  ...
  annotate("text", x = 5, y = 0.55, 
           label = "n = 1: Divergente cuando x tiende a 0", 
...
Genera muestras aleatorias de diferentes tamaños desde 100 hasta 10000 observaciones para las distribuciones ji-cuadrada con n = 5, 20 y 50 grados de libertad. En cada caso:
...
for (df in n) {
  ...
  plot(tamanos, medias_muestrales, 
       type = "l", 
       col = "blue", 
       lwd = 2,
       xlab = "Tamaño de la muestra", 
       ylab = "Media",
       main = paste("Convergencia de la Media (n =", df, ")"))
  abline(h = media_teorica, col = "red", lwd = 2, lty = 2)
  legend("topright", 
         legend = c("Media Muestral", "Media Teórica"), 
         col = c("blue", "red"), 
         lwd = 2, 
         lty = c(1, 2))
  
  plot(tamanos, varianzas_muestrales, 
       type = "l", 
       col = "blue", 
       lwd = 2,
       xlab = "Tamaño de la muestra", 
       ylab = "Varianza",
       main = paste("Convergencia de la Varianza (n =", df, ")"))
  ...
  muestra_grande <- rchisq(10000, df)
  ...
  hist(muestra_grande, 
       ...
       xlab = "Valor", 
       ylab = "Densidad",
       main = paste("Histograma y Densidad Teórica (n =", df, ", n = 10000)"))
  
  x_vals <- seq(0, max(muestra_grande), length.out = 500)
  y_vals <- dchisq(x_vals, df)
  lines(x_vals, y_vals, col = "red", lwd = 2)
  
  legend("topright", 
         legend = c("Histograma", "Densidad Teórica"), 
         col = c("lightblue", "red"), 
         lwd = 2, 
         fill = c("lightblue", NA))
}
...
for (n in tamanos_muestra) {
  ...
  medias_estandarizadas <- (medias - media_chi) / (sd_chi / sqrt(n))
  
  hist(medias_estandarizadas, 
       probability = TRUE, 
       breaks = 50,
       col = "lightblue", 
       border = "white",
       ...
       ylab = "Densidad",
       main = paste("TCL: Medias estandarizadas (n =", n, ", df =", df, ")"))
  ...
  legend("topright", 
         legend = c("Medias estandarizadas", "N(0,1)"), 
         col = c("lightblue", "red"), 
         lwd = 2, 
         fill = c("lightblue", NA),
         bty = "n")
  
  cat("Media de medias estandarizadas:", round(mean(medias_estandarizadas), 4), "\n")
  cat("Desviación estándar:", round(sd(medias_estandarizadas), 4), "\n")
}
```
...
p_sum_normal <- ggplot(df_suma_normal)+
...
for (gl in df) {
  ...
  muestra_chi <- rchisq(num_simulaciones, df = gl)
  muestra_estandarizada <- (muestra_chi - media_chi) / sd_chi
  

  hist(muestra_estandarizada, 
       probability = TRUE, 
       breaks = 50,
       col = "lightblue", 
       border = "white",
       ...
       xlab = "Valor estandarizado", 
       ylab = "Densidad",
       main = paste("Aproximación Normal de Chi-cuadrada (df =", gl, ")"))
  ...
  legend("topright", 
         legend = c("Chi-cuadrada estandarizada", "N(0,1)"), 
         col = c("lightblue", "red"), 
         lwd = 2, 
         fill = c("lightblue", NA))
  
  
  cat("Media estandarizada:", round(mean(muestra_estandarizada), 4), "\n")
  cat("Desviación estándar:", round(sd(muestra_estandarizada), 4), "\n")
  

  qqnorm(muestra_estandarizada, 
         main = paste("Q-Q Plot Normal (df =", gl, ")"))
