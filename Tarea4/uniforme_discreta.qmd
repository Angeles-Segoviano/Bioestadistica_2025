...
x_1 <- 1
x_6 <- 6
valores <- x_1:x_6 #Se hace una secuencia de valores desde el 1 hasta el 6 es un vector
probabilidades <- rep(1/length(valores), length(valores)) #Se repiten las probabilidades para el numero de elementos en el vector valores

# Crear data frame 
dado_data <- data.frame(
  Valor = valores,
  Probabilidad = probabilidades
)#Toma los valores de los vectores hechos anteriormente
...
ggplot(dado_data, aes(x = Valor, y = Probabilidad)) + #De donde sacara los valores, que valor se asigna en cada eje
  geom_col(fill = "steelblue", alpha = 0.7, width = 0.6) + #Color, transparencia y ancho de cada columna
  geom_text(aes(label = round(Probabilidad, 4)), vjust = -0.5) + #las etiquetas de valores en cada columna, el vjust indica un poco arriba del punto
  scale_x_continuous(breaks = valores) + #la escala del eje x
  scale_y_continuous(limits = c(0, 0.20)) + #la escala del eje y
  labs(title = "Distribución Uniforme Discreta - Dado", #el titulo de la grafica
       x = "Valor", #el label del eje x
       y = "Probabilidad") + #el label del eje y
  theme_minimal() + #el tema de la grafica
...
probabilidades <- rep(Probabilidad, length(valores))

# Crear data frame
Urna_data <- data.frame(
  Valor = valores,
  Prob = probabilidades
)
...
# Mostrar tabla
kable(Urna_data, 
      col.names= c("Valor", "Probabilidad"),
      caption = "Distribución de probabilidad de elegir una bola",
      align = "cc") %>%
  kable_styling(bootstrap_options = "striped")

cat("Como es de distribución uniforme discreta, solo debemos de contar cuantos pares hay en la urna ")
cat("Los cuales serían 4: 2, 4, 6 y 8.")

contador <- sum(valores %% 2 == 0)
# Multiplicamos la probabilidad *4

P_pares <- contador * Probabilidad
cat("La probabilidad de obtener un número par es de:", P_pares)

#Crear grafico
ggplot(Urna_data, aes(x = Valor, y = Prob)) + 
  geom_col(fill = "olivedrab3", width = 0.6) + 
  geom_text(aes(label = round(Prob, 4)), vjust = -0.5) +
  scale_x_continuous(breaks = valores) + 
  labs(title = "Distribución de probabilidad de los elementos en la Urna", 
       x = "Valor", 
       y = "Probabilidad") + 
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5))
...
# valores
valores <- 5:15
Prob <- 1/ length(valores)
probabilidades <- rep(Prob, length(valores))
...
# Mostrar tabla
kable(Exr_data, 
      caption = "Distribución de probabilidad del ejercicio",
      align = "cc") %>%
  kable_styling(bootstrap_options = "striped")

media <- sum(valores) * Prob
varianza <- sum((valores-media)^2) * Prob
...
ggplot(Exr_data, aes(x=Valor, y=Probabilidad))+
  geom_col( fill = "purple1")+
  geom_text(aes(label = round(Prob, 4)), vjust = -0.5) +
  scale_x_continuous(breaks = valores) + 
  scale_y_continuous(limits = c(0, Prob + 0.005))+
  labs(title = paste("Distribución de probabilidad del ejercicio"),
       x = "Valor",
       y = "Probabilidad")+ 
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5))
...
media_muestral <- mean(sim_values)
...
cat("Varianza muestral: ", round(varianza_muestral, 3))
...
tabla_frecuencias <- tibble(
  ...
  Frec_observada= as.vector(frecuencias)
)

kable(tabla_frecuencias,
      caption = "Comparación frecuencias",
      col.names = c("Valor", "Frec. Teórica", "Frec. Observada"))|>
  kable_styling(bootstrap_options = "striped")

tabla_frecuencias_long<- tabla_frecuencias|>
  pivot_longer(
    cols = Frec_teorica:Frec_observada,
    names_to = "Frecuencia_tipo",
    values_to = "Frecuencia"
  )

kable(tabla_frecuencias_long,
      caption = "Comparación frecuencias",
      col.names = c("Valor", "Frec. Teórica", "Frec. Observada"))|>
  kable_styling(bootstrap_options = "striped")

ggplot(tabla_frecuencias_long)+
  geom_col(aes(x=Valor, y=Frecuencia, fill=Frecuencia_tipo),
position = "dodge2")+
  scale_fill_manual(values= c("firebrick", "olivedrab3"))+
  labs(fill = "Tipo")+ 
  theme_bw()
...
cat("Media teórica:", (1+6)/2, "\n")
cat("Media observada:", round(mean(medias_muestrales), 4), "\n")
cat("Varianza teórica de la media:",
    round(((6-1+1)^2-1)/12/n_muestra, 4), "\n")
cat("Varianza observada de la media:", round(var(medias_muestrales), 4), "\n")
